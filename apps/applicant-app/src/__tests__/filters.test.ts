...
import { filterNurseries, filterPositions, type Nursery, type Position } from '../utils/filters';
...
describe('Filter Utilities', () => {
  ...
  describe('filterNurseries', () => {
    it('should return all nurseries when search is empty and filter is "all"', () => {
      const result = filterNurseries(mockNurseries, mockPositions, '', 'all');
      expect(result.length).toBe(3);
    });

    it('should filter by nursery name', () => {
      const result = filterNurseries(mockNurseries, mockPositions, 'Sunnyside', 'all');
      expect(result.length).toBe(1);
      expect(result[0].id).toBe(1);
    });

    it('should filter by position title', () => {
      const result = filterNurseries(mockNurseries, mockPositions, 'Manager', 'all');
      expect(result.length).toBe(1);
